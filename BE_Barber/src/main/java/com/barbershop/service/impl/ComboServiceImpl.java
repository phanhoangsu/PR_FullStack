...
//                    // nếu không truyền mô tả thì lấy mô tả mặc định của product
//                    item1.setDescription(item.getDescription() != null ? item.getDescription() : product.getDescription());
//
...
//                    // nếu không truyền mô tả thì lấy mô tả mặc định của service
//                    item1.setDescription(item.getDescription() != null ? item.getDescription() : service.getDescription());
//
//                } else {
...
//            // Nếu là lỗi có mã cụ thể đã được ném bên trong, ném lại y nguyên
//            throw e;
//        } catch (Exception e) {
...
public ComboDetailResponse updateCombo(Integer id, ComboRequest request) {
    ...
    return new ComboDetailResponse(combo.getServiceName(), itemResponses, totalPrice);
}
    ...
    public ComboDetailResponse getComboDetails(Integer id) {
        ...
        return new ComboDetailResponse(combo.getServiceName(), itemResponses, total);
    }
